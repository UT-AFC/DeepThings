...
```
├── darknet-nnpack   -- the git submodule that deepthings is based off of. Runs Neural Network
│   ├── cfg             -- contains configuration scripts for different models
│   ├── data            -- labeling data for the dfiferent images. Also some example images    
|   ├── examples        -- example scripts for running darknet
│   ├── include         -- contains darknet.h include file with important definitions
│   ├── python          -- running darknet with python
│   ├── scripts         -- scripts for some repeated functions
│   └── src             -- source code for darknet
├── data             -- file containing training, testing, and input data for running deepthings                
│   ├── input           -- input images that will run during the example (0.jpg-5.jpg)
│   └── labels          -- image characters and script for drawing labels
├── distriot         -- supporting distributed iot functions for deepthings
│   ├── examples        -- test cases for running the distriot module
│   ├── include         -- darkiot.h file and definitions
│   └── src             -- source code for lower-level network functions, data types, and FIFO queues
├── examples         -- contains single-device running example and accompanying peripherals
│   ├── data            -- data for examples
│   └── profile         -- profile network and gateway performance
├── include          -- contains configure.h file with macro configuration information for deepthings
├── models           -- contains yolo.cfg for YOLOv2 network and should contain yolo.weights
└── src              -- higher level source code for command line processing, running gateway, 
                        running edge devices, performing fused tile partitioning
```
